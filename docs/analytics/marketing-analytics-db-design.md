...
- **Costing model (เจ้าของยืนยัน interim):** `standard_cost = silver_grams × silver_price_today × production_multiplier` · production_multiplier default **1.6** (material + ค่าผลิต ~60%) ต่อ SKU (งานแกะ/ดีไซน์ซับซ้อนตั้งสูงกว่าได้ 2–3×) · **ต้นทุนผันตามราคาเงินโลกรายวัน** → ต้องมีตาราง `ref_silver_price (date, price_per_gram)` refresh รายวัน แล้ว re-cost อัตโนมัติ (ดู margin analysis: `hero-product-margin-cfo.md` §0)
