...
export function SegmentBreakdown({ counts }: { counts: Record<RfmSegment, number> }) {
  const total = RFM_SEGMENTS.reduce((sum, s) => sum + counts[s], 0);

  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3.5 shadow-sm">
      <h3 className="text-sm font-bold text-slate-800">แบ่งกลุ่มลูกค้า (RFM)</h3>
          ...
          return (
            <li key={segment}>
              <Link
                ...
                className="flex flex-col gap-1 rounded-md p-1 hover:bg-slate-50"
              >
