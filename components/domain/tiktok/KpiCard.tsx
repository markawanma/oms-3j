...
export function KpiCard({ kpi }: { kpi: DashboardKpi }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 rounded-lg border border-slate-200 bg-white p-3.5 shadow-sm">
      <span className="text-xs font-medium text-slate-500">{kpi.label}</span>
      <span className="text-xl font-bold tracking-tight text-slate-900 tabular-nums">{kpi.displayValue}</span>
      <div className={`mt-0.5 flex items-center gap-1 text-xs font-semibold ${TREND_CLASS[kpi.trend]}`}>
        ...
        <span className="font-normal text-slate-400">vs เมื่อวาน</span>
      </div>
      {hasCoverageWarning && (
