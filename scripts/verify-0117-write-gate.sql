-- scripts/verify-0117-write-gate.sql
-- Rehearsal + proof for 0117 (missing-orders DB write-gate kill switch)
-- BEFORE Tech Lead applies it for real via apply_migration. Pattern follows
-- scripts/verify-0115-missing-orders.sql / scripts/verify-0111-upsert-
-- rules.sql: STEP 0 replays the migration's DDL byte-identical (verbatim
-- `cat` of supabase/migrations/0117_missing_orders_write_flag.sql below —
-- if you edit that migration, copy the change here too or this rehearsal
-- stops proving what actually ships), STEP 1 is ONE do-block that seeds
-- fixtures, runs every assertion, and ends in `raise exception` so the
-- WHOLE transaction rolls back no matter what (3j-migration-traps #11) —
-- this calls analytics.import_delete_orders/import_restore_orders for
-- real, against analytics.fact_order, the table carrying every real
-- order's revenue.
--
-- shop_id = 'a7c850ee-6776-4c3e-ba72-ba9e8caba2b7' (3J Jewelry, the only
-- real shop_id in this project — same constant verify-0115 uses). Fixtures
-- use the fake order-number prefix 'ZZ' (real data only ever uses ''
-- through 'G', per A3) so they can never collide with real candidate
-- computation, plus a random per-run tag 'VERIFY-0117-<uuid>' on every
-- other identifying field for traceability and so two runs never collide.
--
-- ASSUMES 0112-0116 are ALREADY APPLIED (analytics.fact_order_deleted,
-- import_missing_orders_candidates, import_delete_orders/import_
-- restore_orders as shipped by 0115, etc. all already exist) — this script
-- only replays 0117's OWN DDL (the new analytics.crm_feature_flag table +
-- the two create-or-replace function bodies with the gate spliced in), not
-- the full 0112-0116 chain. If those are somehow missing, STEP 0 fails
-- loudly and immediately (undefined function/table), same as the real
-- apply_migration would.
--
-- Runs every RPC call as service_role (`set_config('request.jwt.claims', ...)`,
-- per 3j-migration-traps #11's own note on testing role-checking functions)
-- — matches exactly how the real app calls these RPCs (getServiceClient()),
-- and means analytics.crm_require_owner_admin's membership check is a
-- deliberate no-op here (same as it is in production right now, pending
-- Auth A2) — this script is testing THIS migration's own new gate, not
-- re-proving crm_require_owner_admin, which 0021's own tests already cover.
--
-- Coverage (see task brief "สิ่งที่ต้องส่ง" §2 for the required-case list):
--   T1        import_delete_orders, no flag row at all          -> reject
--   T-cross   another shop's flag enabled=true, ours has no row -> reject
--             (proves the flag is scoped per-shop, not global)
--   T2        import_delete_orders, flag row enabled=false      -> reject
--   T3        import_delete_orders, flag row enabled=true       -> succeeds
--   T-kill    flip enabled=true -> false mid-session, call again
--             immediately (same transaction, no redeploy)       -> reject
--             (proves this is a REAL kill switch, not just an install-time
--             default)
--   T4        import_restore_orders, no flag row at all         -> reject
--   T5        import_restore_orders, flag row enabled=false     -> reject
--   T6        import_restore_orders, flag row enabled=true      -> succeeds
--   grants    has_function_privilege for anon/authenticated/service_role
--             on BOTH functions, AFTER the create-or-replace above (3j-
--             migration-traps #2 — grants do not survive replace) + a
--             pg_proc row-count check (#1 — no overload from the replace)
--   T0/T8     analytics.fact_order count/sum(revenue) for the real shop,
--             snapshotted before any fixture and after full cleanup, must
--             match exactly
--
-- NOT re-tested here (out of scope for 0117, already covered by verify-
-- 0115-missing-orders.sql and not touched by this migration): the C1-C8
-- candidate-selection rule, P1-P4 preconditions, snapshot/restore field
-- fidelity, merged-customer walk, Shipnity-reuse conflict detection. This
-- script's fixture batch is the SAME minimal "4 boundary rows define the
-- range, 1 extra fact_order row in range = a candidate" shape 0115's own
-- script already proved works, reused here only so import_delete_orders/
-- import_restore_orders have something valid to act on for T3/T6 — the
-- thing actually under test is the gate check itself, not candidate
-- selection.
--
-- ⚠️ Temp shop created for T-cross is dropped in STEP 1's cleanup —
-- confirmed it never gets a row written into analytics.crm_audit_log (the
-- append-only table whose trigger blocks cascading deletes into it,
-- 3j-migration-traps note) because this script never calls anything that
-- writes to crm_audit_log for that shop; it only ever gets a
-- crm_feature_flag row (ON DELETE CASCADE, explicitly deleted first anyway
-- for traceability in the log output).

begin;

-- ============================================================================
-- STEP 0: replay 0117 DDL verbatim (supabase/migrations/0117_missing_orders_write_flag.sql).
-- ============================================================================
...
-- rollback. Pattern: scripts/verify-0111-upsert-rules.sql / verify-0115-
-- missing-orders.sql (T0/T8 golden-snapshot pairing, per-case OK/FAIL log
-- lines, unconditional raise at the end).
-- ============================================================================

do $verify$
declare
  v_shop_id constant uuid := 'a7c850ee-6776-4c3e-ba72-ba9e8caba2b7';
  v_tag text := 'VERIFY-0117-' || replace(gen_random_uuid()::text, '-', '');
  v_log text := E'\n=== verify-0117 write-gate rehearsal ===\n';
  v_fail_count int := 0;
  ...
  v_t8_count bigint;
  v_t8_revenue numeric;

  v_batch uuid;
  v_id_zz950 uuid; -- primary candidate: delete -> restore -> toggle flow
  v_id_zz960 uuid; -- secondary candidate: stays untouched, proves cross-shop + kill-switch rejections are real (nothing silently deleted)
  v_other_shop_id uuid;
  v_deleted_id_zz950 uuid;
  ...
  v_fn_count_delete int;
  v_fn_count_restore int;
  v_priv_anon_delete boolean;
  v_priv_authenticated_delete boolean;
  v_priv_service_role_delete boolean;
  v_priv_anon_restore boolean;
  v_priv_authenticated_restore boolean;
  v_priv_service_role_restore boolean;
  v_priv_anon_table boolean;
  v_priv_authenticated_table boolean;
  v_priv_service_role_table boolean;
begin
  -- run as service_role, matching how the app actually calls these RPCs
  -- (getServiceClient(), see lib/actions/import-missing-orders.ts header).
  perform set_config('request.jwt.claims', '{"role":"service_role"}', true);

  select dca.channel_id, dca.alias_raw into v_chan_id, v_chan_alias
    from analytics.dim_channel_alias dca limit 1;
  if v_chan_id is null then
    raise exception 'verify-0117: need at least 1 analytics.dim_channel_alias row for fixtures -- environment problem, not a 0117 bug. Stop and check seed data.';
  end if;

  -- ==========================================================================
  -- T0 golden snapshot (whole shop, BEFORE any fixture exists).
  -- ==========================================================================
  select count(*), coalesce(sum(revenue), 0) into v_t0_count, v_t0_revenue
    from analytics.fact_order where shop_id = v_shop_id;
  v_log := v_log || format(E'T0 golden: count=%s revenue=%s\n', v_t0_count, v_t0_revenue);

  -- ==========================================================================
  -- fixtures: one batch defining prefix ZZ range [900,999] over
  -- 2026-06-01..2026-06-04 (4 confirmed rows, both stg_order_import AND
  -- fact_order -- same minimal shape verify-0115-missing-orders.sql already
  -- proved works), plus two extra fact_order rows in range/date/channel but
  -- NOT in the batch's file -> both are real C1-C8 candidates. ZZ950 is the
  -- one this script actually deletes/restores; ZZ960 is left untouched
  -- throughout so every "should be rejected" check has something to prove
  -- stayed untouched (T1's own subject, ZZ950, is unusable for that once
  -- T3 legitimately deletes it).
  -- ==========================================================================
  insert into analytics.stg_import_batch (shop_id, source_type, file_name, file_hash, status, imported_at)
  values (v_shop_id, 'excel_order_report', v_tag || '.xlsx', v_tag, 'transformed', now())
  ...
  values
    (v_batch, v_shop_id, '{}'::jsonb, 'excel', 'ZZ900', v_chan_alias, '2026-06-01 09:00:00+07', 100, 0, 'transformed'),
    (v_batch, v_shop_id, '{}'::jsonb, 'excel', 'ZZ910', v_chan_alias, '2026-06-02 09:00:00+07', 100, 0, 'transformed'),
    (v_batch, v_shop_id, '{}'::jsonb, 'excel', 'ZZ990', v_chan_alias, '2026-06-03 09:00:00+07', 100, 0, 'transformed'),
    (v_batch, v_shop_id, '{}'::jsonb, 'excel', 'ZZ999', v_chan_alias, '2026-06-04 09:00:00+07', 100, 0, 'transformed');

  insert into analytics.fact_order (shop_id, source_order_no, channel_id, order_date, revenue)
  values
    (v_shop_id, 'ZZ900', v_chan_id, '2026-06-01', 100),
    (v_shop_id, 'ZZ910', v_chan_id, '2026-06-02', 100),
    (v_shop_id, 'ZZ990', v_chan_id, '2026-06-03', 100),
    (v_shop_id, 'ZZ999', v_chan_id, '2026-06-04', 100);

  insert into analytics.fact_order (shop_id, source_order_no, channel_id, order_date, revenue)
  values (v_shop_id, 'ZZ950', v_chan_id, '2026-06-02', 555.55)
  returning id into v_id_zz950;

  insert into analytics.fact_order (shop_id, source_order_no, channel_id, order_date, revenue)
  values (v_shop_id, 'ZZ960', v_chan_id, '2026-06-02', 222.22)
  returning id into v_id_zz960;

  -- other shop, with ITS OWN flag row set enabled=true from the start -- sits
  -- in place through every "our shop should still be rejected" check below,
  -- proving the flag is scoped per (shop_id, flag) and never leaks across
  -- shops.
  insert into public.shop (name) values (v_tag || ' temp shop') returning id into v_other_shop_id;
  insert into analytics.crm_feature_flag (shop_id, flag, enabled) values (v_other_shop_id, 'missing_orders_write', true);

  -- ==========================================================================
  -- T1: import_delete_orders, NO flag row for our shop at all -> reject.
  -- ==========================================================================
  begin
    perform analytics.import_delete_orders(v_shop_id, v_batch, array[v_id_zz950], v_tag || ' T1');
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T1: delete succeeded with NO flag row for this shop -- gate is not fail-closed\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail, v_msg = message_text;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T1: import_delete_orders rejected, detail=write_gate_closed (no flag row)\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T1: rejected but wrong reason (detail=%s message=%s) -- expected write_gate_closed\n', v_detail, v_msg);
    end if;
  end;
  if not exists (select 1 from analytics.fact_order where id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T1: ZZ950 was deleted despite the gate being closed -- STOP, data safety violated\n';
  end if;
  if exists (select 1 from analytics.fact_order_deleted where fact_order_id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T1: a fact_order_deleted snapshot row exists for ZZ950 despite the rejected delete\n';
  end if;

  -- ==========================================================================
  -- T-cross: another shop's flag row is enabled=true (inserted above) --
  -- must NOT unlock this shop. Our shop still has no row at this point.
  -- ==========================================================================
  begin
    perform analytics.import_delete_orders(v_shop_id, v_batch, array[v_id_zz960], v_tag || ' T-cross');
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T-cross: delete succeeded -- another shop''s enabled=true flag leaked into this shop''s gate\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T-cross: another shop''s enabled=true flag did NOT unlock this shop\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T-cross: rejected but wrong detail=%s\n', v_detail);
    end if;
  end;
  if not exists (select 1 from analytics.fact_order where id = v_id_zz960) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T-cross: ZZ960 was deleted\n';
  end if;

  -- ==========================================================================
  -- T2: import_delete_orders, flag row enabled=false -> reject.
  -- ==========================================================================
  insert into analytics.crm_feature_flag (shop_id, flag, enabled) values (v_shop_id, 'missing_orders_write', false);

  begin
    perform analytics.import_delete_orders(v_shop_id, v_batch, array[v_id_zz950], v_tag || ' T2');
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T2: delete succeeded with flag row enabled=false\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T2: import_delete_orders rejected, enabled=false\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T2: rejected but wrong detail=%s\n', v_detail);
    end if;
  end;
  if not exists (select 1 from analytics.fact_order where id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T2: ZZ950 was deleted despite enabled=false\n';
  end if;

  -- ==========================================================================
  -- T3: import_delete_orders, flag row enabled=true -> succeeds for real.
  -- ==========================================================================
  update analytics.crm_feature_flag set enabled = true, updated_at = now()
    where shop_id = v_shop_id and flag = 'missing_orders_write';

  begin
    select analytics.import_delete_orders(v_shop_id, v_batch, array[v_id_zz950], v_tag || ' T3') into v_delete_result;
    v_log := v_log || format(E'OK   T3: import_delete_orders succeeded with enabled=true, result=%s\n', v_delete_result);
  exception when others then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || format(E'FAIL T3: expected success but got error: %s\n', sqlerrm);
  end;

  if v_delete_result is null or (v_delete_result ->> 'deleted_count')::int is distinct from 1 then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || format(E'FAIL T3: deleted_count=%s, want 1\n', v_delete_result ->> 'deleted_count');
  end if;
  if exists (select 1 from analytics.fact_order where id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T3: ZZ950 still present after a supposedly successful delete\n';
  end if;

  select id into v_deleted_id_zz950 from analytics.fact_order_deleted
    where fact_order_id = v_id_zz950 and restored_at is null;
  if v_deleted_id_zz950 is null then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T3: no fact_order_deleted snapshot row found for ZZ950\n';
  else
    v_log := v_log || E'OK   T3: fact_order_deleted snapshot row created for ZZ950\n';
  end if;

  -- ==========================================================================
  -- T-kill: flip enabled=true -> false MID-SESSION (same transaction, no
  -- redeploy), then call again immediately -> must reject right away. This
  -- is the actual "is it a real kill switch" proof the brief asked for.
  -- ==========================================================================
  update analytics.crm_feature_flag set enabled = false, updated_at = now()
    where shop_id = v_shop_id and flag = 'missing_orders_write';

  begin
    perform analytics.import_delete_orders(v_shop_id, v_batch, array[v_id_zz960], v_tag || ' T-kill');
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T-kill: delete succeeded right after flipping the flag closed -- not a real kill switch\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T-kill: flipping enabled true->false mid-session rejects the very next call\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T-kill: rejected but wrong detail=%s\n', v_detail);
    end if;
  end;
  if not exists (select 1 from analytics.fact_order where id = v_id_zz960) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T-kill: ZZ960 was deleted\n';
  end if;

  -- ==========================================================================
  -- T4: import_restore_orders, NO flag row for our shop -> reject.
  -- ZZ950 (deleted in T3) is still sitting in fact_order_deleted,
  -- restored_at is null, so this is a real restore attempt against a real
  -- row -- exactly what a caller with a valid deleted_id but a closed gate
  -- would hit.
  -- ==========================================================================
  delete from analytics.crm_feature_flag where shop_id = v_shop_id and flag = 'missing_orders_write';

  begin
    perform analytics.import_restore_orders(v_shop_id, array[v_deleted_id_zz950]);
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T4: restore succeeded with NO flag row for this shop\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T4: import_restore_orders rejected, detail=write_gate_closed (no flag row)\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T4: rejected but wrong detail=%s\n', v_detail);
    end if;
  end;
  if exists (select 1 from analytics.fact_order where id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T4: ZZ950 was restored despite the gate being closed\n';
  end if;
  if exists (select 1 from analytics.fact_order_deleted where id = v_deleted_id_zz950 and restored_at is not null) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T4: fact_order_deleted.restored_at got set despite the rejected restore\n';
  end if;
  ...
  begin
    perform analytics.import_restore_orders(v_shop_id, array[v_deleted_id_zz950]);
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T5: restore succeeded with enabled=false\n';
  exception when others then
    get stacked diagnostics v_detail = pg_exception_detail;
    if v_detail = 'write_gate_closed' then
      v_log := v_log || E'OK   T5: import_restore_orders rejected, enabled=false\n';
    else
      v_fail_count := v_fail_count + 1;
      v_log := v_log || format(E'FAIL T5: rejected but wrong detail=%s\n', v_detail);
    end if;
  end;
  if exists (select 1 from analytics.fact_order where id = v_id_zz950) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T5: ZZ950 was restored despite enabled=false\n';
  end if;

  -- ==========================================================================
  -- T6: import_restore_orders, flag row enabled=true -> succeeds for real.
  -- ==========================================================================
  update analytics.crm_feature_flag set enabled = true, updated_at = now()
    where shop_id = v_shop_id and flag = 'missing_orders_write';

  begin
    select analytics.import_restore_orders(v_shop_id, array[v_deleted_id_zz950]) into v_restore_result;
    v_log := v_log || format(E'OK   T6: import_restore_orders succeeded with enabled=true, result=%s\n', v_restore_result);
  exception when others then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || format(E'FAIL T6: expected success but got error: %s\n', sqlerrm);
  end;

  if v_restore_result is null or (v_restore_result ->> 'restored_count')::int is distinct from 1 then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || format(E'FAIL T6: restored_count=%s, want 1\n', v_restore_result ->> 'restored_count');
  end if;
  if not exists (select 1 from analytics.fact_order where id = v_id_zz950 and revenue = 555.55) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T6: ZZ950 not back in fact_order with its original revenue after restore\n';
  end if;
  if not exists (select 1 from analytics.fact_order_deleted where id = v_deleted_id_zz950 and restored_at is not null) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T6: fact_order_deleted.restored_at was not set after a successful restore\n';
  end if;

  -- ==========================================================================
  -- grants + overload check, AFTER the create-or-replace in STEP 0
  -- (3j-migration-traps #1 no-overload, #2 grants-do-not-survive-replace).
  -- ==========================================================================
  select count(*) into v_fn_count_delete from pg_proc
    where pronamespace = 'analytics'::regnamespace and proname = 'import_delete_orders';
  select count(*) into v_fn_count_restore from pg_proc
    where pronamespace = 'analytics'::regnamespace and proname = 'import_restore_orders';
  v_log := v_log || format(E'pg_proc rows after DDL: import_delete_orders=%s import_restore_orders=%s (both must be 1)\n', v_fn_count_delete, v_fn_count_restore);
  if v_fn_count_delete <> 1 then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: import_delete_orders has an overload -- arg list changed silently (3j-migration-traps #1)\n';
  end if;
  if v_fn_count_restore <> 1 then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: import_restore_orders has an overload -- arg list changed silently (3j-migration-traps #1)\n';
  end if;

  select has_function_privilege('anon', 'analytics.import_delete_orders(uuid,uuid,uuid[],text)', 'execute') into v_priv_anon_delete;
  select has_function_privilege('authenticated', 'analytics.import_delete_orders(uuid,uuid,uuid[],text)', 'execute') into v_priv_authenticated_delete;
  select has_function_privilege('service_role', 'analytics.import_delete_orders(uuid,uuid,uuid[],text)', 'execute') into v_priv_service_role_delete;
  select has_function_privilege('anon', 'analytics.import_restore_orders(uuid,uuid[])', 'execute') into v_priv_anon_restore;
  select has_function_privilege('authenticated', 'analytics.import_restore_orders(uuid,uuid[])', 'execute') into v_priv_authenticated_restore;
  select has_function_privilege('service_role', 'analytics.import_restore_orders(uuid,uuid[])', 'execute') into v_priv_service_role_restore;

  v_log := v_log || format(E'grants import_delete_orders: anon=%s authenticated=%s service_role=%s\n', v_priv_anon_delete, v_priv_authenticated_delete, v_priv_service_role_delete);
  v_log := v_log || format(E'grants import_restore_orders: anon=%s authenticated=%s service_role=%s\n', v_priv_anon_restore, v_priv_authenticated_restore, v_priv_service_role_restore);

  if v_priv_anon_delete or v_priv_authenticated_delete or not v_priv_service_role_delete then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: import_delete_orders grants wrong after replace (want anon=f authenticated=f service_role=t)\n';
  else
    v_log := v_log || E'OK   import_delete_orders grants correct after replace\n';
  end if;
  if v_priv_anon_restore or v_priv_authenticated_restore or not v_priv_service_role_restore then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: import_restore_orders grants wrong after replace (want anon=f authenticated=f service_role=t)\n';
  else
    v_log := v_log || E'OK   import_restore_orders grants correct after replace\n';
  end if;

  select has_table_privilege('anon', 'analytics.crm_feature_flag', 'select') into v_priv_anon_table;
  select has_table_privilege('authenticated', 'analytics.crm_feature_flag', 'select') into v_priv_authenticated_table;
  select has_table_privilege('service_role', 'analytics.crm_feature_flag', 'select') into v_priv_service_role_table;
  v_log := v_log || format(E'grants crm_feature_flag(select): anon=%s authenticated=%s service_role=%s\n', v_priv_anon_table, v_priv_authenticated_table, v_priv_service_role_table);
  if v_priv_anon_table or not v_priv_authenticated_table or not v_priv_service_role_table then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: analytics.crm_feature_flag select grants wrong (want anon=f authenticated=t service_role=t)\n';
  else
    v_log := v_log || E'OK   analytics.crm_feature_flag select grants correct\n';
  end if;

  if not exists (
    select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'analytics' and c.relname = 'crm_feature_flag' and c.relrowsecurity
  ) then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL: analytics.crm_feature_flag does not have RLS enabled\n';
  else
    v_log := v_log || E'OK   analytics.crm_feature_flag has RLS enabled\n';
  end if;

  -- ==========================================================================
  -- cleanup every fixture this script created, then re-snapshot (T8).
  -- Temp shop (v_other_shop_id) never wrote to analytics.crm_audit_log
  -- (append-only, cascade-blocking trigger) -- only ever got a
  -- crm_feature_flag row, explicitly deleted below before the shop itself.
  -- ==========================================================================
  ...
  delete from analytics.stg_import_batch where id = v_batch;
  delete from analytics.crm_feature_flag where shop_id = v_shop_id and flag = 'missing_orders_write';
  delete from analytics.crm_feature_flag where shop_id = v_other_shop_id and flag = 'missing_orders_write';
  delete from public.shop where id = v_other_shop_id;

  select count(*), coalesce(sum(revenue), 0) into v_t8_count, v_t8_revenue
    from analytics.fact_order where shop_id = v_shop_id;
  v_log := v_log || format(E'T8 post-cleanup: count=%s revenue=%s\n', v_t8_count, v_t8_revenue);

  if v_t8_count <> v_t0_count or v_t8_revenue is distinct from v_t0_revenue then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || format(E'FAIL T8: post-cleanup snapshot does NOT match T0 (count %s->%s revenue %s->%s) -- a fixture leaked. STOP -- do not ship.\n', v_t0_count, v_t8_count, v_t0_revenue, v_t8_revenue);
  else
    v_log := v_log || E'OK   T8: post-cleanup snapshot matches T0 exactly (count/revenue) -- no fixture leaked\n';
  end if;
  ...
  if exists (select 1 from analytics.crm_feature_flag where shop_id in (v_shop_id, v_other_shop_id) and flag = 'missing_orders_write') then
    v_fail_count := v_fail_count + 1;
    v_log := v_log || E'FAIL T8: a crm_feature_flag fixture row leaked\n';
  end if;
  ...
  if v_fail_count > 0 then
    v_log := v_log || format(E'\n=== %s CHECK(S) FAILED -- 0117 is NOT safe to ship as-is ===\n', v_fail_count);
  else
    v_log := v_log || E'\n=== ALL CHECKS PASSED -- safe to apply 0117 for real (apply_migration) ===\n';
  end if;
