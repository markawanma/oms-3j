-- 0047_fix_profit_status_cast.sql
-- Bug fix in analytics.transform_pending_orders (surfaced re-importing an order
-- report that overlaps an already-imported month — e.g. "shipnity orderAug1-15"
-- over the earlier "1-10 Aug"): the ON CONFLICT DO UPDATE set
--   profit_status = case when ... = 'actual' then 'actual' else 'estimated' end
-- produces `text`, and text -> enum is NOT an implicit cast in an UPDATE SET, so
-- every conflicting (existing) order errored with
--   "column profit_status is of type profit_status_t but expression is of type text".
-- New orders (INSERT path, 'estimated' as an unknown literal) coerced fine, which
-- is why earlier first-time imports never hit it. Fix: cast the CASE result to
-- analytics.profit_status_t. Function body is otherwise byte-identical to 0045
-- (incl. the recompute_is_new_customer(p_shop_id) tail call) — only the ON
-- CONFLICT profit_status expression gains `::analytics.profit_status_t`.
--
-- Applied to the live DB before writing this file (import was blocked); this
-- migration records it for code/DB parity.
--
-- KNOWN, NOT fixed here (separate follow-up): transform_pending_order_lines does
-- not promote a row from 'orphan' back to 'pending' when its fact_order later
-- appears, so re-running it after the order-level import does not auto-close old
-- orphans. Worked around this import by loading the newer authoritative
-- line-item file as a fresh batch (all 'pending').
