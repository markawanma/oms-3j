-- 0045_fix_is_new_customer.sql
-- Fix: analytics.fact_order.is_new_customer computed order-dependently at
-- transform time (per-row "no earlier fact_order for this customer" check)
-- + frozen on conflict (never overwritten). Aug batch was imported
-- (2026-08-11) BEFORE Jan-Jul (2026-08-13) -> at Aug transform time there
-- were no earlier rows in the system yet -> returning customers got flagged
-- "new" and frozen there. Verified impact: 1,188/4,830 orders with a
-- customer_id flagged wrong (currently new=3,841, correct is new=2,653 /
-- returning=2,177).
--
-- Correct, order-independent definition: an order is "new customer" IFF it
-- is that customer's earliest order (order_date asc, id as tiebreak).
-- Orders with customer_id is null stay is_new_customer = null (unknown,
-- untouched by any part of this migration).
--
-- Three parts:
--   (1) analytics.recompute_is_new_customer(p_shop_id) — set-based,
--       order-independent, idempotent (only touches rows whose value is
--       actually wrong) re-derivation for one shop.
--   (2) One-time backfill — run it now for every shop that has orders.
--   (3) create-or-replace analytics.transform_pending_orders — byte-identical
--       to the live definition (pulled via pg_get_functiondef) except ONE
--       added line, `perform analytics.recompute_is_new_customer(p_shop_id);`,
--       right before `return query select v_transformed, v_errored;` — so
--       every future import re-derives the whole shop and import order can
--       never desync this column again. No other logic touched (profit /
--       on-conflict guard / PII handling all verbatim).
--
-- ⚠️ DO NOT APPLY — file only, per task instructions. Tech Lead applies via MCP.

-- ============================================================================
-- (1) analytics.recompute_is_new_customer
-- ============================================================================

create or replace function analytics.recompute_is_new_customer(p_shop_id uuid)
returns void
language plpgsql
security definer
...
begin
  if p_shop_id is null then
    raise exception 'recompute_is_new_customer: p_shop_id is required';
  end if;

  update analytics.fact_order fo
  set is_new_customer = (r.rn = 1),
      updated_at = now()
  from (
    select id,
  ...
  where fo.id = r.id
    and fo.is_new_customer is distinct from (r.rn = 1);
end;
$$;

revoke execute on function analytics.recompute_is_new_customer(uuid) from public, anon, authenticated;
grant execute on function analytics.recompute_is_new_customer(uuid) to service_role;

-- ============================================================================
-- (2) One-time backfill — re-derive is_new_customer for every shop that
-- currently has orders, order-independent so it's safe to run any time.
-- ============================================================================
...
declare
  s uuid;
begin
  for s in select distinct shop_id from analytics.fact_order loop
    perform analytics.recompute_is_new_customer(s);
  end loop;
end;
$$;

-- ============================================================================
-- (3) analytics.transform_pending_orders — verbatim live definition + one
-- added line (marked below) that recomputes is_new_customer for the whole
-- shop at the end of every transform run, so import order can no longer
-- desync this column.
-- ============================================================================
...
begin
  ...
  loop
    begin
      ...
      on conflict (shop_id, source_order_no) do update set
        ...
        profit_status = case when analytics.fact_order.profit_status = 'actual' then 'actual' else 'estimated' end,
  ...
  perform analytics.recompute_is_new_customer(p_shop_id);   -- <== added line (0045): re-derive whole shop, order-independent
  return query select v_transformed, v_errored;
end;
$function$;

-- transform_pending_orders' own execute grant is unchanged by this migration
-- (already granted to service_role only, revoked from public/anon/authenticated
-- since 0013/whichever migration last touched it) — create-or-replace does not
-- reset grants, so no re-grant statement needed here.

-- ============================================================================
-- Verify (example only — commented out, Tech Lead runs manually after apply).
-- ============================================================================

-- -- Expect: new=2653, returning=2177 (matches Tech Lead's manual count).
-- select is_new_customer, count(*)
-- from analytics.fact_order
-- where customer_id is not null
-- group by is_new_customer
-- order by is_new_customer;
--
-- -- Idempotency check: recompute again immediately, should be a no-op
-- -- (compare fact_order.updated_at before/after — none should move).
-- do $$
-- declare v_shop_id uuid;
-- begin
--   for v_shop_id in select distinct shop_id from analytics.fact_order loop
--     perform analytics.recompute_is_new_customer(v_shop_id);
--   end loop;
-- end;
-- $$;
